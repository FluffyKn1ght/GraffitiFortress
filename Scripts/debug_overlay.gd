...
func _process(delta: float) -> void:
	...
	if Global.local_chara:
		...
		%TopRightLabel.text += "\n\nMovement:\nInputs: joy={0} jmp={1} cro={2}\nGround Vel: {3} ({4})\nAir Vel: {5}\nTOTAL VEL: {6}\nFloor: {7}; Wall: {8}; Ceiling: {9}".format([Global.local_chara.joystick, Input.is_action_pressed("Jump"), Global.local_chara.crouch, Global.local_chara.ground_velocity, Global.local_chara.ground_velocity.length(), Global.local_chara.air_velocity, Global.local_chara.velocity, Global.local_chara.is_on_floor(), Global.local_chara.is_on_wall(), Global.local_chara.is_on_ceiling()])
		%TopRightLabel.text += "\n\nFloor materials:\nTexture: {0}\nPhysics: friction={1} bounce={3}{2}".format(["<TODO>", Global.local_chara.floor_phymat.friction, Global.local_chara.floor_phymat.bounce, "-" if Global.local_chara.floor_phymat.absorbent else ""])
